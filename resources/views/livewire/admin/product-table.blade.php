<div class="w-full py-8 px-4 sm:px-6 lg:px-8" x-data="productEditor()">
    {{-- Alertas --}}
    @if (session('success'))
        <script>
            Swal.fire({
            ...
            <table class="w-full">
                <thead class="bg-zinc-800">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-sm font-medium text-zinc-300 uppercase">Precio (S/.)</th>
                ...
                <tbody class="divide-y divide-zinc-800">
                    @foreach ($products as $product)
                        <tr>
                            ...
                            <td class="px-4 py-4 text-sm text-zinc-300">{{ Str::limit($product->descripcion, 50) }}</td>
                            <td class="px-4 py-4 text-sm text-zinc-300">{{ $product->categoria }}</td>
                            <td class="px-4 py-4 text-sm text-zinc-300">{{ $product->stock }}</td>
                            <td class="px-4 py-4 text-sm text-zinc-300">S/. {{ number_format($product->precio, 2) }}</td>
                            <td class="px-4 py-4 text-sm text-right">
                                <button @click="openEditModal({{ $product->id }}, '{{ addslashes($product->nombre) }}', '{{ addslashes($product->descripcion) }}', '{{ $product->categoria }}', {{ $product->stock }}, {{ $product->precio }})"
                                    class="text-blue-500 hover:text-blue-400 mr-3">
                                    <svg class="h-5 w-5" fill="currentColor" viewBox="0 0 20 20">
                                        <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" />
                                    </svg>
                                </button>
                                <button onclick="confirmDelete({{ $product->id }})" class="text-red-500 hover:text-red-400">
                                    <svg class="h-5 w-5" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd" d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v6a1 1 0 102 0V8a1 1 0 00-1-1z" clip-rule="evenodd" />
                                    </svg>
                                </button>
                                <form id="delete-form-{{ $product->id }}" action="{{ route('admin.product.destroy', $product->id) }}" method="POST" class="hidden">
                                    @csrf
                                    @method('DELETE')
                                </form>
                            </td>
    ...
    <template x-teleport="body">
        <div x-show="showEdit" x-cloak class="fixed inset-0 bg-black bg-opacity-60 z-50 flex items-center justify-center">
            <div class="bg-zinc-900 rounded-xl shadow-2xl border border-zinc-700 w-full max-w-2xl p-6">
                ...
                <form :action="`/admin/producto/${editId}`" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label class="text-zinc-300 text-sm">Categoría</label>
                            <input type="text" name="categoria" x-model="editCategoria" required
                                class="w-full bg-zinc-800 text-white px-3 py-2 rounded-lg border border-zinc-600" />
                        </div>
                        <div>
                            <label class="text-zinc-300 text-sm">Stock</label>
                            <input type="number" name="stock" x-model="editStock" min="0" required
                                class="w-full bg-zinc-800 text-white px-3 py-2 rounded-lg border border-zinc-600" />
                        </div>
                        <div>
                            <label class="text-zinc-300 text-sm">Precio (S/.)</label>
                            <input type="number" name="precio" x-model="editPrecio" step="0.01" min="0" required
                                class="w-full bg-zinc-800 text-white px-3 py-2 rounded-lg border border-zinc-600" />
                        </div>
                    </div>
                    <div class="mt-4">
...
<script>
    function productEditor() {
        return {
            ...
            editStock: 0,
            editPrecio: 0,
            openEditModal(id, nombre, descripcion, categoria, stock, precio) {
                this.editId = id;
